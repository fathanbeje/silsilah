<div class="entry {{ $node['children']->count() === 1 ? 'sole' : '' }} {{ !empty($isRoot) ? 'entry-root' : '' }}">
    <div class="tree-node-card">
        <span class="label">{{ link_to_route('users.tree', $node['user']->name, [$node['user']->id], ['title' => $node['user']->name.' ('.$node['user']->gender.')']) }}</span>
        @if (!empty($node['origin_family']) && (!empty($node['origin_family']['spouse']) || !empty($node['origin_family']['is_unmapped'])))
